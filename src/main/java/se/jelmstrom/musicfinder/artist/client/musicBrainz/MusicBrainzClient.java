...
public class MusicBrainzClient {
    ...
    private static final Log LOG = LogFactory.getLog(MusicBrainzClient.class);
    private static final String DISCOGS_TYPE_ID = "04a5b104-a4c2-4bac-99a1-7b837c37d9e4";
    private static final String SEARCH_ARTIST_PATTERN = "https://musicbrainz.org/ws/2/artist?query=%s";
    private static final String GET_ARTIST_PATTERN = "https://musicbrainz.org/ws/2/artist/%s?inc=url-rels+release-groups";


    public ArtistList searchArtist(String query) {

        try {
            String url = String.format(SEARCH_ARTIST_PATTERN, URLEncoder.encode(query, StandardCharsets.UTF_8.toString()));
    ...
    public Artist getArtist(String mbId) {
        Optional<Artist> result = client.makeRequest(String.format(GET_ARTIST_PATTERN, mbId), Artist.class);
        if(result.isPresent()){
